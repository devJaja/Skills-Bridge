...
contract SkillBridge is ReentrancyGuard {
   ...
   //Update user profile information
    ...
    function postJobListing(
        ...
        emit JobListingPosted(jobListingCount, msg.sender);
    }
        ...
        onlyClient 
    {
        ...
        emit JobListingClosed(_listingId);
    }
        ...
        nonReentrant 
    {
        ...
        emit JobFunded(jobCounter, _listingId, msg.sender, msg.value, listing.deadline);
        emit ProviderAssigned(jobCounter, _provider);
        emit JobListingClosed(_listingId);
    }
        ...
        nonReentrant 
    {
        ...
        emit JobFunded(jobCounter, 0, msg.sender, msg.value, _deadline);
        emit ProviderAssigned(jobCounter, _provider);
    }
   ...
   //Cancel funded job (only if provider hasn't started)
    ...
    function applyForJob(uint256 _listingId) external onlyRegistered nonReentrant {
        ...
        // Emit an event for application
        emit JobApplication(msg.sender, _listingId);
    }
